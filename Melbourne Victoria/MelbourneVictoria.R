getwd()
setwd("C:\\Users\\adijo\\Desktop\\NationalCrisis\\Airbnb Datasets\\Melbourne Victoria")
...
# months <- c('apr', 'mar', 'feb', 'jan', 'dec', 'nov', 'oct', 'sept', 'aug', 'jul', 'jun', 'may')

# import melbourne listings into compiled dataframe, remove unwanted columns
listings <- dir('data', full.names = T) %>% map_df(read_csv)
listings_sum <- listings[c(1,4,27,28,30,31,33,40, 56, 74)]
listings_sum$city <- 'Melbourne'
...
########## Imputation ############ fill in missing july
...
monthly_neighbourhood_aggregates$city <- 'melbourne'
...
hist(listings_sum[listings_sum$months=='2020-06-01',]$price, main="Daily Price for Airbnb Listings 06-2020", xlab ="Price ($)",
     ylab="Count")
...
hist(listings_sum[listings_sum$months=='2020-06-01' & listings_sum$price <=upper_bound,]$price, main ="Daily Price for Airbnb Listings 06-2020 (Inliers)", 
...
write.csv(listings_sum_inlier, 'melbourne_listings-inlier.csv') 
write.csv(monthly_aggregates, 'melbourne_monthly_agg_in.csv') 
write.csv(monthly_neighbourhood_aggregates, "melbourne_monthly_neighbourhood_agg_in.csv")
